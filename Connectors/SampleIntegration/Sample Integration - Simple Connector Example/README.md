...
|Password Field|Example of API password field parameter. API doesn't require authentication, this parameter is just for showcase purpose.||*****|
|Currencies To Fetch|Connector will fetch exchange rates for the provided currencies.||USD, EUR|
|Create Alert Per Exchange Rate|If enabled, connector will create a separate Alert per each exchange rate.||false|
|Alert Severity|Possible values: Critical, High, Medium, Low, Informational||Informational|
|Add Attachment|If enabled, connector will add JSON object to the alert.||true|
...
|Use dynamic list as blocklist|If enabled, the dynamic list will be used as a blocklist.||false|
|Disable Overflow|If enabled, connector will ignore the overflow mechanism.||false|
|Verify SSL|If enabled, verify the SSL certificate for the connection to the API Service server is valid.||true|
